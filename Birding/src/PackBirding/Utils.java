...
public class Utils {
	...
	public static void add(Scanner reader, BirdsDB db, Person person) {  
		//Ask for input data
		String name = askForName(reader);
		String nameLatin = askForNameLatin(reader);
		
		String firstName = askForFirstName(reader);
		String surName = askForSurName(reader);
		
		//Create object Bird
		Bird bird = new Bird(name, nameLatin, 0, new Person(firstName, surName));
	...
	public static String askForFirstName(Scanner reader) {
		System.out.println("First name of person: ");
		return reader.nextLine();
	}
	
	public static String askForSurName(Scanner reader) {
		System.out.println("Sur name of person: ");
		return reader.nextLine();
	}
